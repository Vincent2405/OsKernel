...
HeapInfo KernelHeap;//der kernel heap 

//initialisiert x pages vom betriebsystem
void* heapInitPage(int pages,HeapInfo* heap)
{
    //initialisiert den freien speicher
    unsigned long page_size = getpagesize(); 

    //kprint("hole kernelPage",20);
    void* addr=Kernelmmap(0x0,page_size*pages,0,1,0,0);
...
void* memAlloc(unsigned long requestedsize,HeapInfo* heap)
{
    ...
    //aligne auf PAGESIZE
    unsigned long page_size = (unsigned long)getpagesize(); // Hole die Seitengröße des Systems
    unsigned long neededSpace=(unsigned long)sizeof(FreeNode)+requestedsize;
    unsigned long neededPages = (neededSpace+page_size-1)/page_size;//hochgerundet
